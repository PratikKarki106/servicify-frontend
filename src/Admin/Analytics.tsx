...
const DateRangePicker: React.FC<{ dateRange: DateRange; onChange: (range: DateRange) => void }> = ({ dateRange, onChange }) => (
...
const Analytics: React.FC = () => {
  ...
  const analyticsQuery = useQuery({
    queryKey: ['analytics', revenueRange, dateRange.startDate, dateRange.endDate],
    queryFn: () => fetchAnalyticsData(dateRange, revenueRange),
  ...
  const isRefreshing = analyticsQuery.isFetching && !!data;
      ...
      <div className="analytics-container">
        ...
        {isInitialLoading ? (
          ...
          <>
            <div className="kpi-row">
              {[
                { icon: <FaDollarSign />, label: 'Total Revenue', value: `Rs. ${(data!.kpiMetrics.totalRevenue / 1000).toFixed(0)}k`, color: COLORS.primary },
                ...
                { icon: <FaTools />, label: 'Avg Daily Services', value: data!.kpiMetrics.avgDailyServices, color: COLORS.warning },
            ...
            <div className="analytics-card full-width">
              <div className="analytics-card-header">
                <div>
                  ...
                  <p className="analytics-card-subtitle">{revenueRange === 'weekly' ? 'This week vs last week' : 'Monthly revenue for the selected period'}</p>
                </div>
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={data!.revenue as RevenueDataPoint[]} barSize={revenueRange === 'weekly' ? 24 : 18} margin={{ top: 8, right: 12, left: -5, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,0.06)" vertical={false} />
                  <XAxis dataKey={revenueKey} tick={{ fill: '#6c757d', fontSize: 12 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: '#6c757d', fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} />
            ...
            <div className="analytics-two-col">
              <div className="analytics-card">
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <PieChart>
                    <Pie data={data!.status as StatusDataPoint[]} cx="50%" cy="48%" innerRadius={65} outerRadius={100} paddingAngle={3} dataKey="value">
                      {data!.status.map((entry, i) => (
                        <Cell key={i} fill={entry.color || STATUS_COLORS[entry.name] || COLORS.primary} stroke="transparent" />
                      ))}
                    </Pie>
                    <Tooltip formatter={(v, n) => [`${v ?? 0} appointments`, n]} contentStyle={{ background: '#fff', border: '1px solid #e9ecef', borderRadius: 8, fontSize: 13, color: '#333', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} />
                    <Legend iconType="circle" iconSize={8} formatter={(v) => <span style={{ color: '#495057', fontSize: 12 }}>{v}</span>} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="analytics-card">
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <AreaChart data={data!.appointmentsTrend as AppointmentTrendDataPoint[]} margin={{ top: 8, right: 12, left: -15, bottom: 0 }}>
                    ...
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,0.06)" vertical={false} />
                    <XAxis dataKey={data!.appointmentsTrend[0]?.week ? 'week' : 'date'} tick={{ fill: '#6c757d', fontSize: 12 }} axisLine={false} tickLine={false} />
                    ...
                    <Area type="monotone" dataKey="count" stroke={COLORS.secondary} strokeWidth={2.5} fill="url(#apptGrad)" name="Appointments" dot={{ fill: COLORS.secondary, r: 4 }} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
